...
app.controller('myPageCtrl', function($scope,$http){
	...
	$scope.menuScroll = function() {
		...
		$scope.scrollNav = function(e) {
			...
			} else if ($(e.target).hasClass('link_fin')) {
				$scope.pageAnimate('linkelement2');
			} else if ($(e.target).hasClass('link_deb')) {
				$scope.pageAnimate('linkdeb');
			} else if ($(e.target).hasClass('link_sec')) {
				$scope.pageAnimate('linksec');
			} else if ($(e.target).hasClass('link_adh')) {
				$scope.pageAnimate('linkadh');
			} else if ($(e.target).hasClass('link_reu')) {
				$scope.pageAnimate('linkreu');
			} else if ($(e.target).hasClass('link_tem')) {
				$scope.pageAnimate('linktem');
			} else if ($(e.target).hasClass('link_ch')) {
				$scope.pageAnimate('linkch');
			...
			}else if ($(e.target).hasClass('link_pnri')) {
				$scope.pageAnimate('pnrilink');
			}else if ($(e.target).hasClass('link_ges')) {
				$scope.pageAnimate('geslink');
			}else if ($(e.target).hasClass('link_par')) {
				$scope.pageAnimate('parlink');
	...
	$scope.counterInit = function() {
		$('.statistic').on('inview.uk.scrollspy', function(){
			...
			$scope.numAnim2 = new CountUp('statistic_counter_2', 0, 8400, 0, 3.5);
			$scope.numAnim3 = new CountUp('statistic_counter_3', 0, 2300, 0, 3.5);
			$scope.numAnim4 = new CountUp('statistic_counter_4', 0, 54, 0, 3.5);
			$scope.numAnim5 = new CountUp('statistic_counter_5', 0, 10, 0, 3.5);
			$scope.numAnim6 = new CountUp('statistic_counter_6', 0, 3, 0, 3.5);
			$scope.numAnim7 = new CountUp('statistic_counter_7', 0, 9, 0, 3.5);
			$scope.numAnim1.start();
			$scope.numAnim2.start();
			$scope.numAnim3.start();
			$scope.numAnim4.start();
			$scope.numAnim5.start();
			$scope.numAnim6.start();
